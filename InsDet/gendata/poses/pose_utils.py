...
def save_views(realdir, names):
    with open(os.path.join(realdir, 'poses/view_imgs.txt'), mode='w') as f:
        f.writelines('\n'.join(names))
    f.close()
...
def load_save_pose(realdir):
    # load colmap data
    camerasfile = os.path.join(realdir, 'poses/sparse/0/cameras.bin')
    ...
    imagesfile = os.path.join(realdir, 'poses/sparse/0/images.bin')
    ...
    points3dfile = os.path.join(realdir, 'poses/sparse/0/points3D.bin')
    ...
    np.save(os.path.join(realdir, 'poses/poses_bounds.npy'), save_arr)
...
def gen_poses(basedir, match_type, factors=None):
    ...
    if os.path.exists(os.path.join(basedir, 'poses/sparse/0')):
        files_had = os.listdir(os.path.join(basedir, 'poses/sparse/0'))
    else:
        files_had = []
