...
def save_image_and_mask(
    ...
    # リサイズ
    debug_image = image * debug_mask[:, :, np.newaxis]
    # debug_image = cv.addWeighted(debug_image, mask_alpha, image, mask_beta, 0)

    white_bg = np.where(debug_mask == 0, 255, 0).astype("uint8")
    white_bg = np.stack([white_bg, white_bg, white_bg], axis=-1)
    debug_image = white_bg + debug_image

    resize_image = cv.resize(debug_image, output_size)
